...
const cli = program
  .version("0.0.1")
  .description(
    "Momentum Safe MoveScript tools. Run the compiled MOVE script on blockchain."
  )
  .option(
    "-c, --config <string>",
  ...
  .option(
    ...
    "address of momentum safe deployer",
    DEFAULT_MSAFE
  )
...
async function main() {
  ...
  const receiver =
    "0xca4a2fb9c6d811666fedb1e02953ece60f447a63adf8adbe577ff9f359f4273d";
    ...
    msafe,
    {
      ...
      args: [
        new TxnBuilderTypes.TransactionArgumentAddress(
          TxnBuilderTypes.AccountAddress.fromHex(receiver)
        ),
